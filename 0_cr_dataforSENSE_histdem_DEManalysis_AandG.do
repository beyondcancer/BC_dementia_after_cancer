...
use "$datafiles_an_dem/cr_dataforSENSE_histdem_DEManalysis_aurum.dta", replace
append using  "$datafiles_an_dem/cr_dataforSENSE_histdem_DEManalysis_gold.dta"
tab exposed
recode eth5_cprd 5=.
...
save  "$datafiles_an_dem/cr_dataforSENSE_histdem_DEManalysis_AandG.dta", replace
cap log close

********************************************************************************

foreach site of global cancersites {
use	"$datafiles_an_dem/cr_dataforSENSE_histdem_DEManalysis_AandG.dta", clear
keep if cancer=="`site'"
...
tab maxex
drop if max==0
save "$datafiles_an_dem/cr_dataforSENSE_histdem_DEManalysis_AandG_`site'.dta", replace
}
