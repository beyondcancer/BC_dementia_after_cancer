...
keep if intvar=="cprd_db"
keep if level==1
gen sense="cprd_aurum"
rename estimate beta
rename se sebeta
rename cancer cancersite
drop pint
*append sense CPRD Aurum
append using "$results_an_dem\an_Sense_cox-model-estimates_processout_sense.dta"
*APPEND main results_an_dem
append using "$results_an_dem\an_Primary_A2_cox-model-estimates_processout.dta"


keep if outcome=="`outcome'"

drop if ca=="crude"
drop if year=="1" | year=="10" | year=="3" | year=="5"
drop db
drop year
drop ca
...
replace analysis=".  No prior outcome" if sense == "h_o_ever"
*replace analysis=".  Data from 2006 onwards" if sense == "2006_onwards"
replace analysis=".  Definite diagnoses" if sense == "definite"
replace analysis=".  Recent consulters" if sense == "exc_non_"
replace analysis=".  Adj. BMI" if sense == "bmi"
replace analysis=".  CPRD Aurum only" if sense == "cprd_aurum"
replace analysis=".  Adj. history mental illness" if sense == "histoutc"
...
replace order=2 if sense == "cprd_aurum"
replace order=3 if sense == "ethnicit"
replace order=4 if sense == "bmi"
replace order=5 if sense == "h_o_ever"
replace order=6 if sense == "definite"
replace order=7 if sense == "exc_non_"
replace order=8 if sense == "histoutc"
