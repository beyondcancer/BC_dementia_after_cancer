	use "$datafiles/cr_dataforDEManalysis_aurum.dta", clear 
	*keep if cancer=="liv"
...
End date (earliest of:  transfer out date, the practice's last collection date, date of death (ONS data), study-end (2016), a dementia diagnosis, or a second cancer diagnosis, whichever occurred first. Cancer patients could act as potential controls until their first cancer diagnosis.*/
	*Gold
	*keep if cprd_db==0
	*alive and under follow-up at 65 years
	gen date_65=dob+(365.25*65)
	format date_65 %td
	keep if indexdate<date_65 & doendcprdfup>date_65

	tab exposed /*9252 with cancer*/
	
	*exclude those with Cancer diagnosis and dementia within one year of each other
	gen time_to_dx=(main0_datedementia-indexdate)/365.25
...
local year 0
	...
	*12 months follow-up after indexdate
	gen fup=(doexit-doentry)/365.25
	sum fup
	drop if fup<1	
	
	stptime if exposed==1 , per(1000)
	stptime if exposed==0, per(1000) 

	stcox exposed
	*stcox exposed age
	*stcox exposed age_cat
