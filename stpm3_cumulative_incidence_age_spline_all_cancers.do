...
mkspline agespl=age, cubic nk(3) dis
stop
...
save "$results_an_dem/stpm3cumrisk_age_spline_`outcome'_`cancersite'", replace
	}
}
}
end
...
di "** `outcome' **"
stpm3cumpredincidences `outcome' 
}
...
stop
 ...
 "$results_an_dem\stpm3cumrisk_dementiamye.gph" "$results_an_dem\stpm3cumrisk_dementialeu.gph", nocopies ycommon            
graph export "$results_an_dem\stpm3cumrisk_final.emf", as(emf)  replace
