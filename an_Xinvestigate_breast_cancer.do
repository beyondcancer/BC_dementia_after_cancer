*Look at records of breast cancer survivors with dementia diagnosis
...
*6,832 in Aurum
save  "$datafiles\breast_cancer_dementia_aurum.dta", replace
...
merge m:1 e_patid using "$datafiles\breast_cancer_dementia_aurum.dta", keep(match)
bysort e_patid (obsdate): keep if _n==1
keep e_patid obsdate 
gen drugs=1
count
save  "$datafiles\dementia_drugs_aurum.dta", replace /*4,586*/
...
keep if binaryvar=="dementia"
destring e_patid, replace
merge m:1 e_patid using "$datafiles\breast_cancer_dementia_aurum.dta", keep(match) /*5,392*/
bysort e_patid (obsdate): keep if _n==1
keep e_patid obsdate icd
tab icd
gen hes=1
save  "$datafiles\dementia_hes_aurum.dta", replace /*7*/
/*G31. 2 Degeneration of nervous system due to alcohol.
G31.9 Degenerative disease of nervous system, unspecified
*/

merge m:1 e_patid using "$datafiles\breast_cancer_dementia_aurum.dta", keep(match using)
drop _m
