...
foreach outcome in dementia   {
	...
	*Only run models if there are >=10 failures in both groups
	...
	stsplit timeband, at(0 1 4 100)
	recode timeband 4=2
	
	*Accounting for matching
	stcox exposed i.imd5##i.timeband i.smokstatus##i.timeband i.alcohol_prob##i.timeband b_diab##i.timeband b_cvd##i.timeband b_hyp##i.timeband b_ckd##i.timeband b_depression##timeband, strata(set) iterate(1000) 
	if _rc==0 estimates save "$results_an_dem/an_Sense_non_PH_cov_cox-model-estimates_adjusted_`cancersite'_`outcome'_`db'_0", replace	
