...
*** COMBINE FOREST PLOTS
	graph combine ///
			$results_an_dem/forest_depression_stage.gph /// 
			$results_an_dem/forest_anxiety_stage.gph ///
			, col(2) iscale(0.6) imargin (0 0 0 0) graphregion(margin(l=0 r=0) color(white)) plotregion(color(white)) ysize(7)
			graph save "$results_an_dem/Forest_stage_anxdep.gph", replace
			graph export "$results\Forest_stage_anxdep.emf", as(emf) name("Graph") replace

	graph combine ///
			$results_an_dem/forest_selfharm_stage.gph ///
			$results_an_dem/forest_suicide_stage.gph ///	
			, col(2) iscale(0.6) imargin (0 0 0 0) graphregion(margin(l=0 r=0) color(white)) plotregion(color(white)) ysize(7)
			graph save "$results_an_dem/Forest_stage_SHsuic.gph", replace
			graph export "$results\Forest_stage_SHsuic.emf", as(emf) name("Graph") replace			
************************************************************************************
log close
...
use "$results_an_dem/an_Secondary_stage_cox_processout.dta", clear
list in 1/10
keep if outcome=="`outcome'"
...
graph save "$results_an_dem/`cancer'.gph", replace
...
if (("`outcome'"=="vte" & ("`cancer'"=="bre"|"`cancer'"=="col"))|("`outcome'"=="hfcardiomyopathy" & "`cancer'"=="nhl")) {
	...
	graph save "$results_an_dem/`cancer'_`outcome'_`db'", replace
	}
*/
} /*outcome*/
...
/*
graph combine "$results_an_dem/nhl_hfcardiomyopathy" "$results_an_dem/bre_vte" "$results_an_dem/col_vte" , altshrink cols(1) ysize(10)
graph export "$results_an_dem/an_Primary_A2_cox-model-figures-interactions_figure2.emf", replace
graph export "$results_an_dem/an_Primary_A2_cox-model-figures-interactions_figure2.pdf", replace

erase "$results_an_dem/nhl_hfcardiomyopathy.gph"
erase "$results_an_dem/bre_vte.gph"
erase "$results_an_dem/col_vte.gph"
