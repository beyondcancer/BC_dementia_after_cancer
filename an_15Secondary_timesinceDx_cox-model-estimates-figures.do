...
merge 1:1 cancer year using "$results_an_dem/an_Secondary_timesinceDx_cox_absolute_numbers"
drop if _m==2
	for var rateunexp rateexp: replace X = 1000*X 
	gen irboth = string(rateexp, "%3.2f") + "/" + string(rateunexp, "%3.2f")
	gen nboth = string(expfail) + "/" + string(unexpfail)
...
foreach cancer in "Oral cavity (C00-06)"  "Oesophageal (C15)" "Stomach (C16)" "Colorectal (C18-21)" "Liver (C22)"  "Pancreas (C25)" "Lung (C34)" "Malignant melanoma (C43)"  "Breast (C50)"  "Cervix (C53)"  {
	count 
	local n=r(N)+1
	set obs `n'
	...
	*by cancersite: replace sitelabel = "[" + string(nfail) + "]" if _n==1 
	...
	di $headingobs
	replace graphorder=$headingobs if graphorder==. & _n==_N 
	gen siteheading = "{bf:Cancer site (ICD10)}" if graphorder==$headingobs
	gen irheading ="{bf:IR CS/GPC}" if graphorder==$headingobs
	gen hrheading ="{bf:HR (95% CI)}" if graphorder==$headingobs
	gen higherriskheading ="{it:(Higher}" if graphorder==$headingobs
	gen lowerriskheading ="{it:(Lower}" if graphorder==$headingobs
	replace graphorder=$headingobs-1 if graphorder==.
...
/*
	|| scatter graphorder irlabpos, m(i)  mlab(irboth) mlabcol(black) mlabsize(vsmall) mlabposition(9)  ///
	|| scatter graphorder irlabpos, m(i) mlab(irheading) mlabcol(black) mlabsize(vsmall) mlabpos(9) ///
